...
require('./settings');
const fs = require('fs');
const { writeExifImg } = require('./lib/exif'); // pastikan path benar
...
await conn.sendMessage(m.chat, {
  ...
  contextInfo: {
    ...
    externalAdReply: {
      ...
      body: `Soft Magic WhatsApp Assistant 🌸`,
...
🌸 Your Status
➤ (${isCreator ? "👑 Pretty Owner" : isPremium ? "💎 Pink Reseller" : "🎀 Sweet User"})
...
case 'negara': {
    try {
        ...
        // Ambil daftar negara
        const listRes = await fetch('https://restcountries.com/v3.1/all?fields=name,cca2,cca3');
        if (!listRes.ok) return m.reply('🍂 *Gagal mengambil daftar negara.*');
        ...
        // Ambil detail negara
        const detailRes = await fetch(`https://restcountries.com/v3.1/alpha/${code}`);
        if (!detailRes.ok) return m.reply('🍂 *Gagal mengambil detail negara.*');
...
case 'aiclaila':
case 'askclaila':
case 'lightsecretai': {   // ⭐ COMMAND BARU
    try {
        if (!text) {
            return m.reply(`*Contoh:* ${prefix + command} Seberapa pentingnya Rule of Thirds?`);
        }
        ...
        if (!r.ok) return m.reply(`*🍂 Gagal mengambil respons dari server.*`);
...
case 'gpt5':
case 'lightsecretgptai5': {   // ⭐ COMMAND BARU
    try {
        if (!text) {
            return m.reply(`*Contoh:* ${prefix + command} Keunggulan keyboard Titan Elite`);
        }
        ...
        if (!r.ok) {
            return m.reply(`*🍂 Terjadi kesalahan saat menghubungi server.*`);
        }
        ...
        console.log(e);
        m.reply(`*🍂 Terjadi kesalahan internal, coba lagi.*`);
    } finally {
        await conn.sendMessage(m.chat, { react: { text: '', key: m.key } });
    }
}
...
case 'brat': {
  ...
  await conn.sendMessage(m.chat, {
    image: { url: 'https://img1.pixhost.to/images/11043/672776059_nothing_tmp.jpg' },
    caption: `🌸✨ *YAEMIKO • BRAT GENERATOR* ✨🌸

Pilih mode Brat favoritmu di bawah ini ♡
Biarkan Yaemiko mengubah teksmu jadi karya magis ✨

📝 *Teks:* ${text}`,
    footer: 'YAEMIKO-MD • Magical Assistant',
    buttons: [
      { 
        buttonId: `${prefix}brt ${text}`, 
        buttonText: { displayText: "🌸 Brat Image ✨" } 
      },
      { 
...
case "brt": {
    ...
    const mediaUrl = `https://api.hanggts.xyz/imagecreator/brat?text=${encodeURIComponent(text)}`;
    
    await sendStickerUrl(m.chat, mediaUrl, m, {
...
// =======================
case "brt1": {
  if (!text) return m.reply(`✨ *Contoh:* ${prefix + command} nama kamu`);
  ...
  try {
    ...
    const url = `https://api.siputzx.my.id/api/m/brat?text=${encodeURIComponent(text)}&isAnimated=true`;
    const response = await axios.get(url, { responseType: "arraybuffer" });

    const sticker = new Sticker(response.data, {
      pack: "YAEMIKO-MD",
      author: "Magical Assistant",
      type: "full"
    });
    ...
    console.error(err);
    m.reply("❌✨ Gagal membuat Brat Video.\nCoba lagi ya~");
  }
}
break
...
case 'emojimix': {
  if (!text || !text.includes('|'))
    return m.reply(
      `🌸 *EMOJI MIX MAGIC* ✨\n\n` +
      `Gabungkan dua emoji menjadi satu stiker lucu 💕\n\n` +
      `🧪 *Contoh Penggunaan:*\n` +
      `➤ ${prefix + command} 🗿|😴`
    )

  let [emoji1, emoji2] = text.split('|').map(v => v.trim())
  ...
  const mediaUrl =
    `https://restapi-v2.simplebot.my.id/tools/emojimix?emoji1=${encodeURIComponent(emoji1)}&emoji2=${encodeURIComponent(emoji2)}`
  ...
  try {
    ...
    console.error(err)
    m.reply('❌✨ *Gagal memanggil sihir Emoji Mix*')
  }
}
break
...
case 'emojigif': {
  if (!text)
    return m.reply(
      `🌸 *EMOJI GIF MAGIC* ✨\n\n` +
      `Ubah emoji favoritmu jadi stiker GIF lucu 💕\n\n` +
      `🧪 *Contoh Penggunaan:*\n` +
      `➤ ${prefix + command} 😍`
    )
  ...
  const mediaUrl =
    `https://restapi-v2.simplebot.my.id/tools/emojitogif?emoji=${encodeURIComponent(text)}`
  ...
  try {
    ...
    console.error(err)
    m.reply(
      `❌✨ *Emoji gagal dijadikan GIF*\n\n` +
      `🌸 Pastikan emoji valid ya~`
    )
  }
}
break
...
case 'qc': {
    try {
        ...
        if (!teks) return m.reply(`✨ *Contoh:* ${prefix + command} halo semua~`)

        await conn.sendMessage(m.chat, { 
            react: { text: "🌸", key: m.key } 
        })

        const userPp = await conn.profilePictureUrl(
            m.sender, 
            'image'
        ).catch(_ => 'https://telegra.ph/file/0a7151e0e6c4f705fe8c7.jpg')
        ...
        let json = {
            type: "quote",
            format: "png",
            backgroundColor: "#FFEAF3", // soft pink pastel (Yaemiko style)
            width: 512,
            height: 768,
            scale: 2,
            messages: [{
        ...
        const imgPng = Buffer.from(
            hasil.data.result.image, 
            "base64"
        )

        // Convert PNG → WebP
        const webpImg = await sharp(imgPng)
            .webp({ quality: 100 })
            .toBuffer()
        ...
        m.reply(`❌✨ *Gagal membuat QC Sticker Yaemiko.*`)
    }
}
break
...
case 'animeconvert': {
    try {
        ...
        if (!text && !/image\/(jpe?g|png|webp)/.test(mime)) {
            return m.reply(`*Contoh:* ${prefix + command} (Link gambar)\n*Atau reply gambarnya.*`);
        }
...
case 'remini': {
    try {
        ...
        if (!/image\/(jpe?g|png|webp)/.test(mime)) {
            return m.reply(`Reply foto yang ingin di-HD kan`);
        }
        ...
        let api = `https://api.elrayyxml.web.id/api/tools/remini?url=${encodeURIComponent(catboxURL)}`;
        ...
        console.log(e);
        m.reply("❌ Terjadi kesalahan proses HD.");
    }
}
break
...
case 'hapusbg': {
    ...
    // ✅ ENDPOINT BARU
    const resnya = await fetchJson(`https://api.hanggts.xyz/imagecreator/removebg?url=${dd}`);

    await conn.sendMessage(
        m.chat,
        { image: await getBuffer(resnya.result), caption: "Remove Background Done ✅" },
        { quoted: m }
    );
}
break
